...
%Start position
desired=[-0.2, -0.2, 0.5]';
init_t=invKin3D(ls,theta0,desired,n,mode);
...
for iter = 1:n_cans
    ...
    start_pos = start_can_pos(obj_picked+1, :)
    end_pos = end_can_pos(obj_placed+1, :);
    
    for segment = 1:steps
        clf; hold on;
        pos = bezier(starting', start_pos, steps, segment)';
        t = invKin3D(ls, t, pos, n, mode)
    ...
    for segment = 1:steps
        clf; hold on;
        pos = bezier(start_pos,end_pos,steps,segment)';
        t = invKin3D(ls, t, pos, n, mode);
        t = changingTheta(t);
...
end_pos = end_can_pos(obj_placed+1,:);
    for segment = 1:steps
        clf; hold on;
        pos = bezier(end_pos, init_pos', steps, segment)';
        t = invKin3D(ls, t, pos, n, mode);
        [pos_hand, ~] = evalRobot3D(ls, t);
